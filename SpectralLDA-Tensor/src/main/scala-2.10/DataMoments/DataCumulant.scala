...
class DataCumulant(sc: SparkContext, slices: Int, dimK: Int, alpha0: Double, tolerance: Double, documents:RDD[(Long, Double, SparseVector[Double])],dimVocab: Int,numDocs: Long ) extends Serializable {
  ...
  firstOrderMoments = firstOrderMoments.map(x => x/numDocs.toDouble)
  ...
  val (thirdOrderMoments: DenseMatrix[Double], unwhiteningMatrix: DenseMatrix[Double]) = {
    ...
    val whitenedData: RDD[(breeze.linalg.DenseVector[Double], Double)] = documents.map(this_document => (project(dimVocab, dimK, alpha0, eigenValues, eigenVectors, this_document._3), this_document._2))
    var firstOrderMoments_whitened: DenseVector[Double] = whitenedData.map(x => x._1 / x._2).reduce((a, b) => a :+ b)
    ...
    whitenedData.foreach(this_wdoc => Ta += update_thirdOrderMoments(dimK, alpha0, firstOrderMoments_whitened, this_wdoc._1, this_wdoc._2))
    ...
    val unwhiteningMatrix: breeze.linalg.DenseMatrix[Double] = eigenVectors * breeze.linalg.diag(eigenValues.map(x => scala.math.pow(x, 0.5)))
